...
window.logoutAdmin = async function() {
    ...
    try {
        // Reutiliza AuthService.logout() (ya cargado en admin/index.html):
        // cierra sesión en Firebase Y limpia la sesión local que también
        // usa el menú de acceso del sitio público.
        if (window.AuthService && typeof window.AuthService.logout === 'function') {
            await window.AuthService.logout();
        } else {
            // Alternativa por si AuthService no estuviera disponible
        ...
        // Volver siempre a la página de inicio del sitio (no al login del panel)
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.__adminGuardReady) {
        window.__adminGuardReady.then((isVerifiedAdmin) => {
            if (isVerifiedAdmin) syncWithFirebase();
        });
    } else {
        // Página sin guard definido: no arriesgar, no sincronizar.
        console.warn('No se encontró la guarda de administrador; no se cargan datos.');
    }
