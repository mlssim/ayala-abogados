...
// Comprueba contra Firebase (no contra localStorage) que quien llama es un
// administrador autenticado. Ver la nota de seguridad en auth-service.js.
async function requireAdmin() {
    if (window.AuthService && typeof window.AuthService.verifyAdminRole === 'function') {
        return await window.AuthService.verifyAdminRole();
    }
    return false;
}
...
window.AppointmentService = {
    ...
    // Actualizar cita (solo administradores)
    ...
    // Obtener notificaciones (solo administradores)
    ...
    // Marcar notificación como leída (solo administradores)
