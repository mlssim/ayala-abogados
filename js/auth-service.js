...
// IMPORTANTE: session.role (guardado en localStorage/sessionStorage) es solo
// una copia local para pintar la UI al instante. NO es una fuente fiable,
// porque cualquier persona puede editarla desde la consola del navegador.
// Antes de permitir cualquier acción sensible, esta función vuelve a leer el
// rol directamente de Firebase Realtime Database usando el UID autenticado
// real (auth.currentUser), que no se puede falsificar sin credenciales válidas.
// Esto es una capa de defensa en profundidad: la protección definitiva debe
// estar en las Reglas de Seguridad de Firebase (ver database.rules.json).
async function verifyAdminRole() {
    ...
    try {
        ...
        return !!data && data.role === 'administrador' && data.active !== false;
    } catch (e) {
...
window.AuthService = {
    ...
    changeUserRole: async function(userId, newRole) {
        ...
        try {
            ...
            // Si se asigna rol admin, crear nodo en administradores
            if (newRole === 'administrador') {
                await set(ref(db, 'administradores/' + userId), { activo: true });
            } else {
                // CORREGIDO: si se retira el rol admin, revocar también el
                // acceso en administradores. Antes solo se actualizaba
                // usuarios/role, así que un admin degradado seguía
                // pudiendo entrar en admin/login.html indefinidamente.
                await remove(ref(db, 'administradores/' + userId));
            }
    ...
    toggleUserStatus: async function(userId) {
        ...
        try {
            ...
            // Mantener sincronizado el nodo administradores (defensa en
            // profundidad, además de la comprobación de 'active' en login()).
    ...
    deleteUser: async function(userId) {
        ...
        try {
            ...
            await remove(ref(db, 'administradores/' + userId));
            // NOTA: esto solo borra el perfil en Realtime Database. La
            // cuenta de Firebase Authentication de ese usuario NO se puede
            // eliminar desde el cliente (por seguridad, el SDK solo permite
            // borrar la cuenta de la persona que ha iniciado sesión). Para
            // borrar cuentas de otras personas de verdad hace falta una
            // Cloud Function con el Admin SDK. Ver SECURITY.md.
    ...
    // Verificación real de administrador (para uso externo, ej. guardas de página)
    verifyAdminRole: verifyAdminRole
};

// NOTA DE SEGURIDAD:
// Se ha retirado la creación automática de un administrador por defecto
// (antes se ejecutaba en cada carga de página y dejaba la contraseña
// "Admin123!" impresa en la consola del navegador). Para dar de alta al
// primer administrador: registre una cuenta normal desde el sitio y cambie
// manualmente su campo "role" a "administrador" en la consola de Firebase
// (Realtime Database > usuarios > <uid> > role). Ver SECURITY.md.

// Escuchar cambios de autenticación
onAuthStateChanged(auth, (user) => {
    if (user) {
        console.log('Sesión de Firebase Auth activa.');
    } else {
        console.log('No hay sesión de Firebase Auth activa.');
    }
});
